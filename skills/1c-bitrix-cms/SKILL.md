...
- Право исполнять: чтение/диагностика — везде; обратимая запись — в `/local` под git; необратимое/привилегированное (обновления, восстановление, откат) — не на проде по умолчанию (инструкция человеку или изолированная копия). Подробно — `../../shared/kb/conventions.md` («0. Право исполнять»).
...
> База знаний самодостаточна. Ссылки `[src]` и блоки «Источники» — это указание происхождения факта, а не материал для чтения: по умолчанию их не открывай. Обращайся к внешнему первоисточнику только когда нужен версионно-зависимый факт, которого в базе нет, и не обходи вложенные ссылки рекурсивно.
