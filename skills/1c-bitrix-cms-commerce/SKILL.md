...
## База знаний
Каталог/заказ — `../../shared/kb/api-map.md` (commerce-строки); инфоблоки — под-скилл `1c-bitrix-cms-content`. ⚠️ заказ собирается объектами `\Bitrix\Sale\Order`/`Basket` и сохраняется `$order->save()` (не прямой SQL); проверять `Result::isSuccess()`.
