...
## База знаний
Жизненный цикл страницы и шаблона — `../../shared/kb/00-overview.md`; «задача → класс» — `../../shared/kb/api-map.md`. Вывод контента внутри шаблона — под-скилл `1c-bitrix-cms-content`. Каталог штатных компонентов (`menu`/`breadcrumb` и др., их параметры и что копировать в `/local`) — `../../shared/kb/components/00-index.md` (не грузить целиком, брать нужную карточку).
