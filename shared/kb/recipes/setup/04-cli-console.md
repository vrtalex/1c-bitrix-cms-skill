...
`messenger:consume` для scaffolding кастомной логики, генерации
ORM-аннотаций и обновлений модулей из скриптов CI/деплоя.

> **Область применения.** Консольный слой (`bitrix.php` на Symfony Console) —
> для **разработки и обслуживания уже установленного сайта**: scaffolding,
> аннотации ORM, обновления модулей и версий, кеш, индексация, воркеры.
> Это **не инструмент первичной установки**: команды полной установки сайта в
> CLI нет, точка входа `bitrix.php` обслуживает уже развёрнутое ядро.
> Первичную установку ведёт мастер или restore — см.
> [03-installation.md](03-installation.md). Резервное копирование выполняет не
> консольный слой, а отдельный механизм бэкапа — см.
> [../deploy/03-backup-restore.md](../deploy/03-backup-restore.md).
...
> **Резервное копирование — не команда консольного слоя.** Дамп БД и файлов
> делает отдельный механизм: `php -f "$DOCROOT/bitrix/modules/main/tools/backup.php"`
> или мастер «Резервное копирование» в админке. Подробности и восстановление —
> [../deploy/03-backup-restore.md](../deploy/03-backup-restore.md).
...
- [03-installation.md](03-installation.md) — оркестрация установки; CLI здесь — для уже установленного сайта, не для первичной установки.
- [../deploy/03-backup-restore.md](../deploy/03-backup-restore.md) — резервное копирование и восстановление (отдельный механизм, не консольный слой).
