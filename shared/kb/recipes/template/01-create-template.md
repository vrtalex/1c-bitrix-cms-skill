...
2. Создайте `header.php` — верхнюю обёртку: guard, `<!DOCTYPE>`, `<head>` с обязательными `ShowHead()` и `ShowTitle()`, открытие `<body>` с `ShowPanel()`, шапку (логотип/меню/хлебные крошки) и ОТКРЫТЫЙ контейнер контента. Заканчивается незакрытыми тегами.
...
5. Создайте `styles.css` (стили контента) и `template_styles.css` (основной CSS шаблона). Оба подключаются ядром автоматически (`Asset::addTemplateCss()`), `<link>` на них в `header.php` не нужен. При совпадении правил `template_styles.css` грузится после и потому приоритетнее.
6. Создайте `.styles.php` — массив именованных стилей для выпадающего списка «Стиль» визуального HTML-редактора. Если стилей нет — файл возвращает `false`. Это НЕ CSS-файл.
