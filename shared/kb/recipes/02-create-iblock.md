...
// Примечание: при запуске из CLI задать $_SERVER['DOCUMENT_ROOT'] (и 'SERVER_NAME') ДО require,
// иначе ядро не резолвит корень. Подробнее — conventions.md, §3 «CLI-bootstrap».
...
- База знаний: `../api-map.md` (модель iblock, две версии API, `compileEntity`, права; привязка к сайту через `b_iblock_site`, мультисайт `SITE_ID`/`LID`).
```
